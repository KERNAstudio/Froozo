...
function Card({ c, i }: { c: MenuCategory; i: number }) {
  ...
  return (
    ...
    >
      ...
      <span className="absolute right-4 top-4 z-10 text-xs font-semibold text-cream/60">
        {String(i + 1).padStart(2, "0")}
      </span>
      <span className="relative z-10 font-display text-4xl italic leading-none">{c.title}</span>
      {c.note && (
        <span className="relative z-10 mt-2 line-clamp-2 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-cream/70">
          {c.note}
        </span>
      )}
    </Link>
  ...
  return (
    <div className="mt-12 flex items-center gap-2 px-5">
      <button
        ...
        onClick={() => nudge(-1)}
        className="hidden shrink-0 rounded-sm border border-cream/20 p-3 text-cream/70 hover:border-brass hover:text-brass sm:block"
      >
